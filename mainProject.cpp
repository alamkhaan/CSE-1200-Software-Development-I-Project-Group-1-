...
void iKeyboard(unsigned char key)
{
	...
	{
		if(coverImage==16 && powerRun2 == true && !keyType)
		{
			//iPauseTimer(power_timer2);
		...
		if(coverImage==16 && powerRun1 ==false && !keyType)
		{
			//iPauseTimer(power_timer1);
			powerRun2 = true;
			powerRun1 = true;
			redPower = true;
			gk_dive = rand()%7;
	...
	{
		  //boolean variable playerRun will true to start the run of a player
	}

	else if (key == 'p'  && keyType) //default value will store again
	{
			gkRun = true;
			ballMoveX =673;
			ballMoveY =168;
...
int main()
{
	...
	power_timer1 = iSetTimer(80,powerRun1Timer);
	power_timer2 = iSetTimer(5,powerRun2Timer); 
	...
	ball_timer = iSetTimer(80, ballTimer);			  //playerTimer Function is defined in gameFunctions.h hedaer file and is used to change the ball co-ordinates
	gk_dive_timer = iSetTimer(300, gkRunning);		  //gkRunning  Function is defined in gameFunctions.h hedaer file and is used to make diving for Goal Kepper
	victory_timer = iSetTimer(350, victoryRunning);
